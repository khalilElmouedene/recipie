...
class User(Base):
    ...
    # JSON array of reusable Pin Designer elements saved by the user.
    custom_pin_elements: Mapped[str | None] = mapped_column(Text, nullable=True)
...
class Job(Base):
    ...
    email_notification_sent_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=_utcnow)
