...
const IMAGE_HEADER_HINTS = new Set([
  "image", "image_url", "imageurl", "img", "img_url",
  "photo", "photo_url", "picture", "picture_url", "source_image", "source_image_url", "url",
]);
const RECIPE_HEADER_HINTS = new Set([
  "recipe", "recipe_name", "recipe_text", "recipe_title",
  "text", "title", "content", "prompt",
]);

// ─── Types ────────────────────────────────────────────────────────────────────
interface Cell {
  v?: string; b?: boolean; i?: boolean; u?: boolean; s?: boolean;
  fg?: string; bg?: string; ha?: "l" | "c" | "r"; fs?: number;
}
interface SheetData {
...
interface SheetTab { id: string; name: string; data: SheetData; }
interface Workbook { sheets: SheetTab[]; activeId: string; }
interface Selection { row: number; col: number; }
...
interface ColResizeState { col: number; startX: number; startWidth: number; }
interface SheetCtxMenu { x: number; y: number; sheetId: string; }
type SelectionGenerationPreview =
  | { ok: true; rangeLabel: string; items: SharedRecipeInput[]; rowIndices: number[]; sourceRows: number; skippedRows: number; imageCol: number; recipeCol: number; usedHeaderRow: boolean; }
  | { ok: false; rangeLabel: string; message: string; };
interface SelectionCtxMenu { x: number; y: number; preview: SelectionGenerationPreview; }
...
const normalizeRange = (range: CellRange): CellRange => ({
  startRow: Math.min(range.startRow, range.endRow), startCol: Math.min(range.startCol, range.endCol),
  endRow: Math.max(range.startRow, range.endRow), endCol: Math.max(range.startCol, range.endCol),
});
const singleCellRange = (row: number, col: number): CellRange => ({ startRow: row, startCol: col, endRow: row, endCol: col });
...
const emptySheetData = (): SheetData => ({ cells: {}, colWidths: {}, rowHeights: {}, rows: DEFAULT_ROWS, cols: DEFAULT_COLS });
const newTab = (name: string): SheetTab => ({ id: uid(), name, data: emptySheetData() });
const emptyWorkbook = (): Workbook => { const t = newTab("Sheet1"); return { sheets: [t], activeId: t.id }; };

function parseStored(raw: string): Workbook {
  try {
    const parsed = JSON.parse(raw);
    if (parsed.sheets && parsed.activeId) return parsed as Workbook;
    if (parsed.cells !== undefined) { const t: SheetTab = { id: uid(), name: "Sheet1", data: parsed as SheetData }; return { sheets: [t], activeId: t.id }; }
  } catch {}
...
function sheetDataToCsv(sheet: SheetData): string {
  ...
  const rowCount = Math.max(1, maxRow + 1); const colCount = Math.max(1, maxCol + 1);
  const escape = (value: string) => `"${value.replace(/"/g, "\"\"")}"`;
  const lines: string[] = [];
  for (let r = 0; r < rowCount; r++) {
    const rowValues: string[] = [];
    for (let c = 0; c < colCount; c++) rowValues.push(sheet.cells[cellKey(r, c)]?.v ?? "");
...
function csvToRows(text: string): string[][] {
  ...
  for (let i = 0; i < source.length; i++) {
    const ch = source[i];
    if (inQuotes) {
      if (ch === "\"") { if (source[i + 1] === "\"") { current += "\""; i++; } else { inQuotes = false; } }
      else current += ch;
      continue;
    }
...
function rowsToSheetData(rows2d: string[][]): SheetData {
  ...
  safeRows.forEach((row, r) => { row.forEach((val, c) => { if (val !== "") data.cells[cellKey(r, c)] = { v: val }; }); });
  return data;
}
...
const isLikelyImageValue = (value: string): boolean => {
  ...
  return v.startsWith("http://") || v.startsWith("https://") || v.startsWith("/uploads/") ||
    /^data:image\//.test(v) || /\.(png|jpe?g|webp|gif|avif|svg)(\?.*)?$/.test(v);
};

const scoreImageColumn = (values: string[]): number =>
  values.reduce((sum, value) => { const v = value.trim(); if (!v) return sum; if (isLikelyImageValue(v)) return sum + 3; if (v.includes("/")) return sum + 1; return sum; }, 0);

const scoreRecipeColumn = (values: string[]): number =>
  values.reduce((sum, value) => { const v = value.trim(); if (!v) return sum; if (isLikelyImageValue(v)) return sum; if (v.length >= 20) return sum + 3; if (v.includes(" ")) return sum + 2; return sum + 1; }, 0);
...
function clampFloatingCardPosition(x: number, y: number) {
  ...
  const clampedY = fitsBelow ? y : Math.max(12, y - FLOATING_CARD_HEIGHT);
...
function buildSelectionGenerationPreview(sheet: SheetData, range: CellRange): SelectionGenerationPreview {
  ...
  if (width < 2) return { ok: false, rangeLabel, message: "Select at least two columns so Auto Spy can map image_url and recipe_text." };
  const selectedRows = Array.from({ length: normalized.endRow - normalized.startRow + 1 }, (_, rowOffset) => {
    ...
    const values = Array.from({ length: width }, (_, colOffset) => { const col = normalized.startCol + colOffset; return sheet.cells[cellKey(row, col)]?.v?.trim() ?? ""; });
    return { row, values };
  }).filter((entry) => entry.values.some(Boolean));
  if (!selectedRows.length) return { ok: false, rangeLabel, message: "The selected range is empty." };
  ...
  let imageIndex = -1; let recipeIndex = -1; let usedHeaderRow = false; let dataRows = selectedRows;
  if (imageHeaderIndex !== -1 && recipeHeaderIndex !== -1 && imageHeaderIndex !== recipeHeaderIndex) {
    imageIndex = imageHeaderIndex; recipeIndex = recipeHeaderIndex; usedHeaderRow = true; dataRows = selectedRows.slice(1);
  } else if (width === 2) {
    ...
    if (secondImageScore > firstImageScore) { imageIndex = 1; recipeIndex = 0; }
    else if (firstImageScore > secondImageScore) { imageIndex = 0; recipeIndex = 1; }
    else {
      const firstRecipeScore = scoreRecipeColumn(firstColumnValues);
      const secondRecipeScore = scoreRecipeColumn(secondColumnValues);
      if (firstRecipeScore > secondRecipeScore) { imageIndex = 1; recipeIndex = 0; }
      else { imageIndex = 0; recipeIndex = 1; }
    }
  } else {
    return { ok: false, rangeLabel, message: "Select exactly two columns, or include header names like image_url and recipe_text in the selected block." };
  }
  if (!dataRows.length) return { ok: false, rangeLabel, message: "I found the column mapping, but there are no data rows under that selection yet." };
  let skippedRows = 0; const items: SharedRecipeInput[] = []; const rowIndices: number[] = [];
  dataRows.forEach(({ row, values }) => {
    const image_url = values[imageIndex]?.trim() ?? "";
    const recipe_text = values[recipeIndex]?.trim() ?? "";
    if (image_url && recipe_text) { items.push({ image_url, recipe_text }); rowIndices.push(row); return; }
    if (values.some(Boolean)) skippedRows += 1;
  });
  if (!items.length) return { ok: false, rangeLabel, message: "No valid rows were found. Each selected row needs both an image URL and recipe text." };
  return { ok: true, rangeLabel, items, rowIndices, sourceRows: dataRows.length, skippedRows, imageCol: normalized.startCol + imageIndex, recipeCol: normalized.startCol + recipeIndex, usedHeaderRow };
}
...
export default function AutoSpyPage() {
  ...
  const activeSheet = workbook.sheets.find((s) => s.id === workbook.activeId) ?? workbook.sheets[0];
  ...
  const clampToSheet = useCallback((row: number, col: number): Selection => ({
    row: clampIndex(row, 0, sheetRows - 1), col: clampIndex(col, 0, sheetCols - 1),
  }), [sheetRows, sheetCols]);

  const setSingleSelection = useCallback((row: number, col: number) => {
    const bounded = clampToSheet(row, col);
    setSel(bounded); setSelectionAnchor(bounded);
  ...
  const selectedCellsFromRanges = useCallback((ranges: CellRange[]): Selection[] => {
    const seen = new Set<string>(); const out: Selection[] = [];
    ranges.forEach((range) => {
      const n = normalizeRange(range);
      const startRow = clampIndex(n.startRow, 0, sheetRows - 1); const endRow = clampIndex(n.endRow, 0, sheetRows - 1);
      const startCol = clampIndex(n.startCol, 0, sheetCols - 1); const endCol = clampIndex(n.endCol, 0, sheetCols - 1);
      for (let r = startRow; r <= endRow; r++) for (let c = startCol; c <= endCol; c++) {
        const key = cellKey(r, c);
        if (seen.has(key)) continue;
        seen.add(key); out.push({ row: r, col: c });
      }
    });
    return out;
  }, [sheetRows, sheetCols]);
  ...
  useEffect(() => { selectionRangesRef.current = selectionRanges; }, [selectionRanges]);

  useEffect(() => {
    if (!tabCtxMenu && !selectionCtxMenu) return;
    const close = () => { setTabCtxMenu(null); setSelectionCtxMenu(null); };
  ...
  const scheduleAutoSave = useCallback((wb: Workbook) => {
    ...
    autoSaveTimer.current = setTimeout(() => {
      api.saveAutoSpySheet(id, JSON.stringify(wb))
  ...
  const updateWorkbook = useCallback((updater: (prev: Workbook) => Workbook) => {
    setWorkbook((prev) => { const next = updater(prev); scheduleAutoSave(next); return next; });
  }, [scheduleAutoSave]);

  const updateActiveData = useCallback((updater: (prev: SheetData) => SheetData) => {
    updateWorkbook((wb) => ({ ...wb, sheets: wb.sheets.map((s) => s.id === wb.activeId ? { ...s, data: updater(s.data) } : s) }));
  }, [updateWorkbook]);

  useEffect(() => {
    const resetResizeCursor = () => { if (typeof document === "undefined") return; document.body.style.cursor = ""; document.body.style.userSelect = ""; };
    const handleResizeMove = (e: MouseEvent) => {
      const resize = colResizeRef.current; if (!resize) return;
    ...
    const handleResizeUp = () => { if (!colResizeRef.current) return; colResizeRef.current = null; resetResizeCursor(); };
    window.addEventListener("mousemove", handleResizeMove);
    window.addEventListener("mouseup", handleResizeUp);
    return () => { window.removeEventListener("mousemove", handleResizeMove); window.removeEventListener("mouseup", handleResizeUp); resetResizeCursor(); };
  }, [updateActiveData]);
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      const res = await api.saveAutoSpySheet(id, JSON.stringify(workbook));
  ...
  // ── Cell helpers ──
  const getCell = (r: number, c: number): Cell => activeSheet?.data.cells[cellKey(r, c)] ?? {};
  const setCell = (r: number, c: number, patch: Partial<Cell>) => {
    updateActiveData((prev) => {
      ...
      if (Object.keys(merged).length === 0) { const { [key]: _, ...rest } = prev.cells; return { ...prev, cells: rest }; }
  ...
  const applyPatchToSelection = useCallback((patch: Partial<Cell>) => {
    ...
    updateActiveData((prev) => {
      ...
      targets.forEach(({ row, col }) => {
        ...
        const merged = { ...existing, ...patch };
        if (merged.v === "") delete merged.v;
        if (Object.keys(merged).length === 0) { delete nextCells[key]; return; }
  ...
  const isSelectedCell = useCallback((row: number, col: number): boolean =>
    selectionRanges.some((range) => isCellInRange(row, col, range)), [selectionRanges]);
  const isSelectedRow = useCallback((row: number): boolean =>
    selectionRanges.some((range) => { const n = normalizeRange(range); return row >= n.startRow && row <= n.endRow; }), [selectionRanges]);
  const isSelectedCol = useCallback((col: number): boolean =>
    selectionRanges.some((range) => { const n = normalizeRange(range); return col >= n.startCol && col <= n.endCol; }), [selectionRanges]);
  ...
  const readClipboardText = useCallback(async (): Promise<string> => {
    if (typeof navigator === "undefined" || !navigator.clipboard?.readText) return clipboardFallbackRef.current;
    try { const text = await navigator.clipboard.readText(); return text || clipboardFallbackRef.current; }
    catch { return clipboardFallbackRef.current; }
  }, []);

  const serializeSelectionToTsv = useCallback((range: CellRange): string => {
    const n = normalizeRange(range); const lines: string[] = [];
    for (let r = n.startRow; r <= n.endRow; r++) {
      const rowVals: string[] = [];
      for (let c = n.startCol; c <= n.endCol; c++) rowVals.push(getCell(r, c).v ?? "");
    ...
    updateActiveData((prev) => {
      ...
      if (oneCellPaste && selectedTargets.length > 1) {
        ...
        matrix.forEach((rowVals, rOffset) => {
          rowVals.forEach((value, cOffset) => { setValue(sel.row + rOffset, sel.col + cOffset, value); });
        });
      }
      return { ...prev, cells: nextCells };
    });
  ...
  const startEdit = (r: number, c: number, initialChar?: string) => {
    ...
    setEditKey(key); setEditVal(val); setFormulaVal(val);
  ...
  const commitEdit = useCallback((moveRow = 0, moveCol = 0) => {
    ...
    setCell(r, c, { v: editVal }); setEditKey(null);
    setSingleSelection(r + moveRow, c + moveCol);
  }, [editKey, editVal, setSingleSelection]); // eslint-disable-line

  const toggleProp = (prop: keyof Cell) => { const cell = getCell(sel.row, sel.col); applyPatchToSelection({ [prop]: !cell[prop as "b"] } as Partial<Cell>); };
  const setProp = (prop: keyof Cell, val: unknown) => applyPatchToSelection({ [prop]: val } as Partial<Cell>);

  const moveSelectionBy = (dRow: number, dCol: number, extend: boolean) => {
    const next = clampToSheet(sel.row + dRow, sel.col + dCol);
    setSel(next);
    if (extend) { setSelectionRanges([buildRange(selectionAnchor, next)]); return; }
    setSelectionAnchor(next); setSelectionRanges([singleCellRange(next.row, next.col)]);
  };

  const selectAllCells = useCallback(() => {
    const origin = { row: 0, col: 0 }; setSel(origin); setSelectionAnchor(origin);
  ...
  const buildRowSelectionRange = useCallback((fromRow: number, toRow: number): CellRange =>
    buildRange({ row: fromRow, col: 0 }, { row: toRow, col: sheetCols - 1 }), [sheetCols]);
  const buildColSelectionRange = useCallback((fromCol: number, toCol: number): CellRange =>
    buildRange({ row: 0, col: fromCol }, { row: sheetRows - 1, col: toCol }), [sheetRows]);

  const handleGridKeyDown = (e: React.KeyboardEvent) => {
    if (editKey !== null) return;
    const ctrlOrMeta = e.ctrlKey || e.metaKey; const key = e.key.toLowerCase();
  ...
  const handleCellMouseDown = (e: React.MouseEvent<HTMLTableCellElement>, row: number, col: number) => {
    ...
    if (editKey !== null) commitEdit(); gridRef.current?.focus();
    const cell = clampToSheet(row, col); const additive = e.ctrlKey || e.metaKey;
    const anchor = e.shiftKey ? selectionAnchor : cell; const baseRanges = additive ? selectionRangesRef.current : [];
    const nextRange = buildRange(anchor, cell);
    setSel(cell); if (!e.shiftKey) setSelectionAnchor(cell);
  ...
  const handleCellMouseEnter = (e: React.MouseEvent<HTMLTableCellElement>, row: number, col: number) => {
    if ((e.buttons & 1) !== 1) return; if (colResizeRef.current) return;
    const drag = dragSelectionRef.current; if (!drag || drag.mode !== "cells") return;
    const bounded = clampToSheet(row, col); const nextRange = buildRange(drag.origin, bounded);
    setSel(bounded); setSelectionRanges(drag.additive ? [...drag.baseRanges, nextRange] : [nextRange]);
  };

  const handleRowHeaderMouseDown = (e: React.MouseEvent<HTMLTableCellElement>, row: number) => {
    if (e.button !== 0) return;
    if (editKey !== null) commitEdit(); gridRef.current?.focus();
    const additive = e.ctrlKey || e.metaKey; const baseRanges = additive ? selectionRangesRef.current : [];
    const boundedRow = clampIndex(row, 0, sheetRows - 1);
    const anchor = e.shiftKey ? selectionAnchor : { row: boundedRow, col: 0 };
    const nextRange = buildRowSelectionRange(anchor.row, boundedRow);
    setSel({ row: boundedRow, col: 0 }); if (!e.shiftKey) setSelectionAnchor({ row: boundedRow, col: 0 });
    setSelectionRanges(additive ? [...baseRanges, nextRange] : [nextRange]);
    dragSelectionRef.current = { mode: "rows", originRow: anchor.row, additive, baseRanges };
    e.preventDefault();
  };

  const handleRowHeaderMouseEnter = (e: React.MouseEvent<HTMLTableCellElement>, row: number) => {
    if ((e.buttons & 1) !== 1) return; if (colResizeRef.current) return;
    const drag = dragSelectionRef.current; if (!drag || drag.mode !== "rows") return;
    const boundedRow = clampIndex(row, 0, sheetRows - 1);
    const nextRange = buildRowSelectionRange(drag.originRow, boundedRow);
    setSel({ row: boundedRow, col: 0 }); setSelectionRanges(drag.additive ? [...drag.baseRanges, nextRange] : [nextRange]);
  };

  const startColResize = (col: number, clientX: number) => {
    if (typeof document !== "undefined") { document.body.style.cursor = "col-resize"; document.body.style.userSelect = "none"; }
    colResizeRef.current = { col, startX: clientX, startWidth: (activeSheet?.data.colWidths[col] ?? DEFAULT_COL_WIDTH) };
  };

  const handleColHeaderMouseDown = (e: React.MouseEvent<HTMLTableCellElement>, col: number) => {
    if (e.button !== 0) return;
    const rect = e.currentTarget.getBoundingClientRect();
    if (e.clientX > rect.right - COL_RESIZE_HITBOX) { startColResize(col, e.clientX); e.preventDefault(); return; }
    if (editKey !== null) commitEdit(); gridRef.current?.focus();
    const additive = e.ctrlKey || e.metaKey; const baseRanges = additive ? selectionRangesRef.current : [];
    const boundedCol = clampIndex(col, 0, sheetCols - 1);
    const anchor = e.shiftKey ? selectionAnchor : { row: 0, col: boundedCol };
    const nextRange = buildColSelectionRange(anchor.col, boundedCol);
    setSel({ row: 0, col: boundedCol }); if (!e.shiftKey) setSelectionAnchor({ row: 0, col: boundedCol });
    setSelectionRanges(additive ? [...baseRanges, nextRange] : [nextRange]);
    dragSelectionRef.current = { mode: "cols", originCol: anchor.col, additive, baseRanges };
    e.preventDefault();
  };

  const handleColHeaderMouseEnter = (e: React.MouseEvent<HTMLTableCellElement>, col: number) => {
    if ((e.buttons & 1) !== 1) return; if (colResizeRef.current) return;
    const drag = dragSelectionRef.current; if (!drag || drag.mode !== "cols") return;
    const boundedCol = clampIndex(col, 0, sheetCols - 1);
    const nextRange = buildColSelectionRange(drag.originCol, boundedCol);
    setSel({ row: 0, col: boundedCol }); setSelectionRanges(drag.additive ? [...drag.baseRanges, nextRange] : [nextRange]);
  };

  const handleColResizeMouseDown = (e: React.MouseEvent<HTMLDivElement>, col: number) => {
    if (e.button !== 0) return; e.preventDefault(); e.stopPropagation();
    if (editKey !== null) commitEdit(); startColResize(col, e.clientX);
  };
  ...
  const handleGridTouchStart = useCallback((e: React.TouchEvent) => {
    ...
    e.preventDefault();
  }, [editKey, commitEdit, clampToSheet, sheetRows, buildRowSelectionRange, getCellFromPoint]);
  ...
  const handleGridTouchMove = useCallback((e: React.TouchEvent) => {
    ...
    e.preventDefault();
  }, [clampToSheet, sheetRows, buildRowSelectionRange, getCellFromPoint]);
  ...
  const findSelectedRangeAt = useCallback((row: number, col: number): CellRange | null => {
    const ranges = selectionRangesRef.current;
    for (let i = ranges.length - 1; i >= 0; i -= 1) { if (isCellInRange(row, col, ranges[i])) return normalizeRange(ranges[i]); }
    return null;
  }, []);
  ...
  const handleCellContextMenu = (e: React.MouseEvent<HTMLTableCellElement>, row: number, col: number) => {
    ...
    if (!findSelectedRangeAt(bounded.row, bounded.col)) { setSel(bounded); setSelectionAnchor(bounded); setSelectionRanges([range]); }
  ...
  const handleRowHeaderContextMenu = (e: React.MouseEvent<HTMLTableCellElement>, row: number) => {
    ...
    if (!findSelectedRangeAt(boundedRow, 0)) { setSel({ row: boundedRow, col: 0 }); setSelectionAnchor({ row: boundedRow, col: 0 }); setSelectionRanges([range]); }
    openSelectionGenerateMenu(e.clientX, e.clientY, range);
  };
  ...
  const handleColHeaderContextMenu = (e: React.MouseEvent<HTMLTableCellElement>, col: number) => {
    ...
    if (!findSelectedRangeAt(0, boundedCol)) { setSel({ row: 0, col: boundedCol }); setSelectionAnchor({ row: 0, col: boundedCol }); setSelectionRanges([range]); }
    openSelectionGenerateMenu(e.clientX, e.clientY, range);
  };
  ...
  const addSheet = () => {
    ...
    let name = `Sheet${n}`; while (existing.includes(name)) name = `Sheet${++n}`;
    const t = newTab(name);
    updateWorkbook((wb) => ({ sheets: [...wb.sheets, t], activeId: t.id }));
    setSingleSelection(0, 0); setEditKey(null);
  };
  ...
  const duplicateSheet = (sheetId: string) => {
    const src = workbook.sheets.find((s) => s.id === sheetId); if (!src) return;
  ...
  const startRename = (sheetId: string) => {
    const tab = workbook.sheets.find((s) => s.id === sheetId); if (!tab) return;
    setRenamingId(sheetId); setRenameVal(tab.name);
  ...
  const commitRename = () => {
    ...
    updateWorkbook((wb) => ({ ...wb, sheets: wb.sheets.map((s) => s.id === renamingId ? { ...s, name } : s) }));
    setRenamingId(null);
  };
  ...
  const handleImport = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (ev) => {
      ...
      const rows = csvToRows(text); const imported = rowsToSheetData(rows);
      setWorkbook((prev) => {
        const next: Workbook = { ...prev, sheets: prev.sheets.map((s) => s.id === prev.activeId ? { ...s, data: imported } : s) };
        scheduleAutoSave(next); return next;
      });
      setSingleSelection(0, 0); setEditKey(null);
    };
  ...
  const handleExport = () => {
    ...
    a.download = `auto-spy-${new Date().toISOString().slice(0, 10)}.csv`;
    document.body.appendChild(a); a.click(); a.remove(); URL.revokeObjectURL(url);
  };

  // ── Auto Spy source handlers ──
  const handleAddSource = async () => {
    const url = urlInput.trim();
    if (!url) return;
    setAddingSource(true);
    try {
      const source = await api.addAutoSpySource(id, url);
      setSources((prev) => [...prev, source]);
      setUrlInput("");
      // Reload workbook since backend added a tab
      const sheetRes = await api.getAutoSpySheet(id);
      if (sheetRes.data) setWorkbook(parseStored(sheetRes.data));
      toast.success(`Added ${source.site_name} — scanning for new posts…`);
    } catch (err) {
  ...
  const handleDeleteSource = async (sourceId: string) => {
    try {
      await api.deleteAutoSpySource(id, sourceId);
  ...
  const handleScanSource = async (sourceId: string) => {
    ...
    try {
      await api.triggerAutoSpyScan(id, sourceId);
      toast.success("Scan triggered — new posts will appear in the sheet shortly.");
      // Reload sheet after a short delay to pick up the scan results
      setTimeout(async () => {
        try {
          const sheetRes = await api.getAutoSpySheet(id);
          if (sheetRes.data) setWorkbook(parseStored(sheetRes.data));
          const sourcesRes = await api.getAutoSpySources(id);
          setSources(sourcesRes);
        } catch {}
      }, 3000);
  ...
  const tableMinWidth = HEADER_WIDTH + Array.from({ length: sheet.cols }, (_, c) => sheet.colWidths[c] ?? DEFAULT_COL_WIDTH).reduce((sum, width) => sum + width, 0);
  ...
  const selectionLabel = selectionRanges.length === 1 ? rangeToAddress(selectionRanges[0]) : `${selectionRanges.length} ranges (${selectedCells.length} cells)`;
    ...
    <div className="flex flex-col h-screen bg-gray-950 overflow-hidden select-none">
      ...
      <div className="shrink-0 border-b border-gray-800 bg-gray-900">
        ...
        <div className="flex items-center gap-2 px-3 py-2 border-b border-gray-800">
          ...
          <span className="text-sm font-semibold text-white ml-1">Auto Spy</span>
          ...
          <button onClick={() => void handleSave()} disabled={saving} className="flex items-center gap-1.5 rounded-md bg-teal-600 px-3 py-1.5 text-xs font-semibold text-white hover:bg-teal-500 disabled:opacity-50 transition">
        ...
        <div className="flex items-center gap-2 border-t border-gray-800 px-3 py-1.5">
          <span className="w-12 shrink-0 rounded border border-gray-700 bg-gray-800 px-2 py-1 text-center text-xs font-mono text-gray-300">{selAddr}</span>
          <input
            value={formulaVal}
            onChange={(e) => {
            ...
            onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); commitEdit(1, 0); gridRef.current?.focus(); } }}
            placeholder="Cell value…"
            className="flex-1 rounded border border-gray-700 bg-gray-800 px-2.5 py-1 text-xs text-gray-100 placeholder-gray-600 focus:border-teal-500 focus:outline-none transition font-mono"
          />
        </div>
      </div>
      ...
      <div
        ...
        onCopy={(e) => { if (editKey !== null) return; e.preventDefault(); void handleCopySelection(); }}
        onPaste={(e) => {
          if (editKey !== null) return;
          const text = e.clipboardData.getData("text/plain");
          if (!text) return; e.preventDefault(); pasteTextAtSelection(text);
        }}
          ...
          <thead>
            <tr style={{ height: HEADER_HEIGHT }}>
              <th
                ...
                onMouseDown={(e) => { if (e.button !== 0) return; gridRef.current?.focus(); if (editKey !== null) commitEdit(); selectAllCells(); e.preventDefault(); }}
              />
              {Array.from({ length: sheet.cols }, (_, c) => (
                ...
                >
                  {colLabel(c)}
                  <div
                    className="absolute right-0 top-0 h-full w-2 cursor-col-resize hover:bg-teal-500/30"
            ...
            {Array.from({ length: sheet.rows }, (_, r) => (
              <tr key={r} style={{ height: sheet.rowHeights[r] ?? DEFAULT_ROW_HEIGHT }}>
                <td
                  data-row={r}
                  data-rowheader="1"
                  className={`sticky left-0 z-10 border-b border-r border-gray-700 bg-gray-800 text-center text-[11px] text-gray-500 cursor-pointer ${isSelectedRow(r) ? "bg-teal-900/30 text-teal-300 font-semibold" : ""}`}
                ...
                {Array.from({ length: sheet.cols }, (_, c) => {
                  ...
                  const cellStyle: React.CSSProperties = {
                    backgroundColor: cell.bg || undefined, color: cell.fg || undefined,
                    fontWeight: cell.b ? "bold" : undefined, fontStyle: cell.i ? "italic" : undefined,
                    textDecoration: [cell.u && "underline", cell.s && "line-through"].filter(Boolean).join(" ") || undefined,
                    textAlign: alignMap[cell.ha ?? "l"] ?? "left", fontSize: cell.fs ? `${cell.fs}px` : "13px",
                  };
                  return (
                    <td
                      key={c}
                      ...
                      className={`relative border-b border-r border-gray-800 px-1.5 overflow-hidden whitespace-nowrap text-gray-100 cursor-default transition-colors ${
                        isActiveCell ? "outline outline-2 outline-teal-500 outline-offset-[-1px] bg-teal-950/20"
                          : isSelected ? "outline outline-1 outline-teal-700/80 outline-offset-[-1px] bg-teal-950/10"
            ...
            return (
              <div
                ...
                className={`group relative flex items-center shrink-0 border-r border-gray-700 px-3 cursor-pointer select-none transition-colors ${
                  isActive ? "bg-gray-950 border-t-2 border-t-teal-500 text-white" : "bg-gray-900 text-gray-400 hover:bg-gray-800 hover:text-gray-200"
                }`}
                style={{ minWidth: 80, maxWidth: 180 }}
                onClick={() => !isRenaming && switchSheet(tab.id)}
                onDoubleClick={() => startRename(tab.id)}
                onContextMenu={(e) => { e.preventDefault(); setSelectionCtxMenu(null); setTabCtxMenu({ x: e.clientX, y: e.clientY, sheetId: tab.id }); }}
              >
                {isRenaming ? (
                  <input
                    ref={renameInputRef}
                    ...
                    className="w-full bg-transparent text-xs text-white outline-none border-b border-teal-400"
                    style={{ minWidth: 60 }}
                  />
                ) : (
      ...
      {tabCtxMenu && (
        ...
        >
          ...
          <CtxItem icon={<Trash2 size={13} />} label="Delete" danger disabled={workbook.sheets.length === 1} onClick={() => { deleteSheet(tabCtxMenu.sheetId); setTabCtxMenu(null); }} />
        </div>
      )}
...
function FmtBtn({ active, title, onClick, children }: { active?: boolean; title?: string; onClick?: () => void; children: React.ReactNode }) {
  return (
    <button title={title} onClick={onClick} className={`flex items-center gap-0.5 h-7 min-w-7 px-1.5 rounded text-xs transition ${active ? "bg-teal-600/30 text-teal-300" : "text-gray-400 hover:bg-gray-700 hover:text-gray-200"}`}>
      {children}
    </button>
  );
}
