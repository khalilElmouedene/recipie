  ...
  const generatePreviews = useCallback(
    async (template: PinTemplate) => {
      ...
      for (let i = 0; i < frames.length; i++) {
        ...
        try {
          const FC =
            (fabricMod as any).Canvas || (fabricMod as any).default?.Canvas;
  ...
  const handleSaveAll = async () => {
    ...
    for (let i = 0; i < frames.length; i++) {
      ...
      if (dataUrl) {
        ...
        a.href = dataUrl;
        a.download = `pin-${String(i + 1).padStart(2, "0")}-${frame.title
          .replace(/[^a-z0-9]/gi, "_")
          .slice(0, 30)}.png`;
  ...
  return (
    <div className="fixed inset-0 z-50 bg-gray-950 flex flex-col">
      ...
      <div className="flex flex-1 min-h-0">
        {/* Template Sidebar */}
        <aside className="w-64 border-r border-gray-800 flex flex-col flex-shrink-0 bg-gray-950 overflow-y-auto">
          ...
          <div className="p-3 space-y-3 overflow-y-auto flex-1">
            <p className="text-xs text-gray-500">
              Select a template to apply to all recipes:
            </p>
            {TEMPLATES.map((t) => (
              <button
                key={t.id}
          ...
          {!selectedTemplate ? (
            <div className="flex flex-col items-center justify-center h-full text-center">
              ...
              <p className="text-gray-600 text-sm mt-1">
                Choose a template from the left panel to preview all your
                recipes
              </p>
            </div>
          ) : (
          ...
          {generating && (
            <div className="flex items-center justify-center gap-2 mt-6">
              ...
              <span className="text-sm text-gray-400">
                Generating previews for {frames.length} recipes...
              </span>
            </div>
          )}
        </main>
