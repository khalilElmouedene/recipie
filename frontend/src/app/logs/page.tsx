...
export default function LogsPage() {
  ...
  // Keep track of current offset for appending pages
  const offsetRef = useRef(0);
  const hasMore = logs.length < total;

  // Resets the list and fetches from offset 0 (used on search / filter change)
  ...
  }, [loadingMore, hasMore, action, tableName, entityPk]);

  const sentinelRef = useInfiniteScroll(loadMoreLogs, { hasMore, loading: loadingMore });
    ...
    <div className="space-y-4">
      ...
      <div className="overflow-hidden rounded-xl border border-gray-800 bg-gray-900">
        ...
        <div className="flex items-center justify-between border-t border-gray-800 px-3 py-2 text-xs text-gray-400">
          <span>Total: {_total} · Chargés: {logs.length}</span>
          {loadingMore && (
        ...
        {/* Sentinel outside the scrollable table so IntersectionObserver picks it up */}
