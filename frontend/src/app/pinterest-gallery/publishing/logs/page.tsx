...
function LogsPage() {
  ...
  useEffect(() => { setBeforeId(undefined); setPublisher(null); setLogs([]); }, [siteId, itemId]);
  useEffect(() => {
    ...
    const refresh = async () => {
      ...
      loading = true;
      try {
        const [status, page] = await Promise.all([api.getPinterestPublisher(siteId), api.getPinterestPublishingLogs(siteId, beforeId, itemId || undefined)]);
  ...
  }, [allowed, siteId, itemId, beforeId]);
  ...
  return <main className="mx-auto max-w-6xl px-4 py-8 sm:px-8">
    ...
    <header className="mt-6 flex flex-wrap items-end justify-between gap-5 border-b border-gray-800 pb-7">
      <div><p className="text-xs font-semibold uppercase tracking-[0.22em] text-red-400">Pinterest / Activity</p><h1 className="mt-3 text-3xl font-semibold tracking-tight">{itemId ? "Item publishing logs" : "Publishing logs"}</h1><p className="mt-3 text-gray-400">{publisher?.domain || "Loading website…"}</p>{itemId && <p className="mt-2 break-all text-xs text-gray-500">Worksheet item {itemId}</p>}</div>
    ...
    <section className="mt-6 overflow-hidden rounded-2xl border border-gray-800">
      ...
      {logs.length === 0 ? <p className="p-10 text-center text-sm text-gray-500">{updated ? (itemId ? "No events recorded for this item yet." : "No events recorded yet. Start publishing from the settings page to follow its progress here.") : "Loading logs…"}</p> : <ol className="divide-y divide-gray-800">{logs.map((log) => <li key={log.id} className="grid gap-3 p-5 sm:grid-cols-[170px_1fr]">
